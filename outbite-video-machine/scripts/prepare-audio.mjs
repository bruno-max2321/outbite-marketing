 ...
 *
 * Expects raw recordings in public/audio/raw/line-0N.* and writes
 * public/audio/line-0N.wav via ffmpeg.
 */
...
if (!existsSync(rawDir)) {
  ...
  console.log(
    "Drop raw VO takes as line-01.* … line-13.*, then re-run npm run prepare-audio",
  );
  process.exit(0);
}

const files = readdirSync(rawDir).filter((f) => /^line-\d{2}\./i.test(f));
if (files.length === 0) {
  console.log("No line-01..line-13 files in public/audio/raw/ yet.");
  console.log("Record VO (see public/audio/README.md), then re-run.");
  process.exit(0);
}

for (const file of files) {
  const id = file.match(/^(line-\d{2})/i)?.[1].toLowerCase();
  if (!id) continue;
